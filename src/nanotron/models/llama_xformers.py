...
from nanotron.nn import flash_attn_bert_padding as bert_padding
...
from nanotron.nn.flash_attn_rotary import RotaryEmbedding as FlashRotaryEmbedding
...
class XFormersAttention(nn.Module):
    ...
    def forward(
        ...
        # TODO GQA backward has a problem, xformers does not support the backward op for GQA
        # query_states: batch_size, seq_len, num_q_heads, head_dim
        # key_states: batch_size, seq_len, num_kv_heads, head_dim
        # value_states: batch_size, seq_len, num_kv_heads, head_dim
        # batch_size, seq_len, num_q_heads, head_dim = query_states.shape
        # num_kv_heads = key_states.shape[-2]
        # if self.is_gqa:
        #     new_shape = [batch_size, seq_len, num_kv_heads, num_q_heads//num_kv_heads, head_dim]
        #     query_states = query_states.reshape(new_shape)
        #     key_states = key_states.reshape([batch_size, seq_len, num_kv_heads, 1, head_dim]).expand(new_shape)
        #     value_states = value_states.reshape([batch_size, seq_len, num_kv_heads, 1, head_dim]).expand(new_shape)
...
class CausalSelfAttention(nn.Module, AttachableStore):
    ...
    def forward(
        ...
        # print('0 qkv_states shape', qkv_states.shape)
        ...
        # print('1 query_states', query_states.shape)
        # print('1 key_states', key_states.shape)
        # print('1 value_states', value_states.shape)
        ...
        if store is not None:  # Inference case
            ...
            # print('2 query_states', query_states.shape)
            # print('2 key_states', key_states.shape)
            # print('2 value_states', value_states.shape)
