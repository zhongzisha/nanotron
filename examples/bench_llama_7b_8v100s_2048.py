...
tokens = TokensArgs(sequence_length=8192, train_steps=80, micro_batch_size=1, batch_accumulation_per_replica=2048)

dataset = PretrainDatasetsArgs(hf_dataset_or_datasets="togethercomputer/RedPajama-Data-1T-Sample", text_column_name="text")

checkpoints_path = os.path.dirname(os.path.dirname(__file__)) + "/checkpoints_2048"
...
config = Config(
    ...
    model=ModelArgs(init_method=RandomInit(std=0.025), model_config=model_config, dtype="float16"),
    tokenizer=TokenizerArgs("meta-llama/Llama-2-7b-hf"),
    optimizer=optimizer,
    logging=LoggingArgs(),
    tokens=tokens,
    data=DataArgs(dataset=dataset, seed=seed, num_loading_workers=2),
    profiler=None,
)
...
if __name__ == "__main__":
    ...
    config.save_as_yaml(f"{dir}/config_llama_7B_8v100s_2048.yaml")

    # Launch training
    os.system("export CUDA_DEVICE_MAX_CONNECTIONS=1")
    gpus = config.parallelism.dp * config.parallelism.pp * config.parallelism.tp
    os.system(f"torchrun --nproc_per_node={gpus} run_train.py --config-file {dir}/config_llama_7B_8v100s_2048.yaml")
